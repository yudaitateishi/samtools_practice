...
for nuc_position in mpileup_list:
	...
	if int(cover_depth) < int(depth_filter): #Filter by cover depth
		continue
	...
	indel_split = re.split("\+|-",markstrip_aln) #strip InDel
	indel_out = []
	...
	snpdict["N"] = aln_text.count("n") + aln_text.count("N")
	...
	if 0.25 < mismatch_avg:
		for k,v in snpdict.items():
			if 4 <= v and 0.25 < round(v / float(cover_depth),3):
				snp.append(k)
	...
	snp_list.append("\t".join([chrom,str(position),str(position),reference,",".join(snp),cover_depth,",".join(mismatch_count)]))
